...
app.post("/stripe/charge", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error al procesar el pago", error });
  }
});
...
async function getStripeKey(claveSucursal) {
  ...
  try {
    ...
    if (res.rows.length > 0) {
      ...
      throw new Error(
        "No se encontró la sucursal o no tiene clave de Stripe configurada"
      );
    }
  } catch (err) {
...
app.listen(port, () =>
  console.log("API CHPSystem Stripe Server started on port", port)
);
