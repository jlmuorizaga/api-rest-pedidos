...
const DB_HOST =
  process.env.DB_HOST || "chp202304db.c2fc27t7knic.us-east-2.rds.amazonaws.com";
...
const insertaCliente = (req, res) => {
  const { id, correoElectronico, contrasenia, nombre, telefono, domicilio } =
    req.body;
  pool.query(
    ...
    (error, results) => {
      ...
      textoRespuesta =
        '{"respuesta": "Se insertó cliente: ' + results.rows[0].id + '"}';
...
const actualizaCliente = (req, res) => {
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      textoRespuesta =
        '{"respuesta": "Se actualizó cliente: ' + results.rows[0].id + '"}';
      res.status(201).json(JSON.parse(textoRespuesta));
    }
  );
};
...
const insertaDatosPedido = (req, res) => {
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      textoRespuesta =
        '{"respuesta": "Se insertó pedido", "id": "' +
        results.rows[0].id +
        '", "numero_pedido": ' +
...
const updateEstatusRecibido = (request, response) => {
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      mensaje =
        '{"mensaje": "Se modificó estatus del pedido",' +
        ' "id": "' +
        results.rows[0].id +
        '",' +
        ' "numero_pedido":' +
